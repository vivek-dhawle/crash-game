...
export class BetPanel {
  ...
  constructor(
    ...
    this.betLabel = new Text({
      text: 'TOTAL BET',
      style: { fontFamily: 'Arial', fill: 0x9ca3af, fontSize: 12, fontWeight: '600' },
    });

    this.betValueText = new Text({
      text: '$10',
      style: { fontFamily: 'Arial', fill: 0xffffff, fontSize: 22, fontWeight: 'bold' },
    });
    this.betValueText.anchor.set(0.5);

    // Buttons
    this.btnMinus = this.createCircleBtn('-', this.colorInput, 0xffffff, () => this.updateAmount(-10));
    this.btnPlus = this.createCircleBtn('+', this.colorInput, 0xffffff, () => this.updateAmount(10));
  ...
  private createCircleBtn(label: string, color: number, textColor: number, onClick: () => void): Container {
  ...
  resize(width: number, height: number) {
    ...
    this.bg.roundRect(inputCenterX - 35, centerY - 20, 70, 40, 8).fill(this.colorInput);
